...
function NoticesRoot() {
  ...
  const [userNoticesOpen, setUserNoticesOpen] = useState(
    Array(userNotices.length).fill(false)
  );
  const [followingNoticesOpen, setFollowingrNoticesOpen] = useState(
    Array(followingNotices.length).fill(false)
  );
        ...
        <div className={styles["notice-list"]}>
          {isMyPage ? (
            isFormOpen ? (
              ...
              <GreenBtn
                className={styles["regist-button"]}
                onClick={() => {
                  setIsFormOpen((prev) => !prev);
                }}
              >
              ...
              userNotices.map((notice) => (
                <div className={styles["each-notice"]}>
                  <div className={styles["notice-title-nickname"]}>
                    ...
                    >
                      {notice.noticeTime[0]}.
                      {notice.noticeTime[1] < 10
                        ? "0" + notice.noticeTime[1]
                        : notice.noticeTime[1]}
                      .
                      {notice.noticeTime[2] < 10
                        ? "0" + notice.noticeTime[2]
                        : notice.noticeTime[2]}
                      &nbsp; &nbsp; {notice.noticeTitle}
                    </div>
              ...
              <div className={styles["each-notice"]}>
                <div className={styles["notice-title-nickname"]}>
                  <div
                    className={styles["notice-title"]}
                    onClick={() =>
                      handleFollowingNoticeDetail(notice.noticeSeq)
                    }
                  >
                    {notice.noticeTime[0]}.
                    {notice.noticeTime[1] < 10
                  ...
                  <Link
                    to={`/${notice.nickname}@${notice.userSeq}`}
                    className={styles["link"]}
                  >
                    <div userSeq={notice.userSeq}>{notice.nickname}</div>
                  </Link>
                </div>
  ...
  return (
    <div>
      <div>
        ...
        <TabContent>{noticesMenuData[noticeIndex].content}</TabContent>
      </div>
    </div>
  );
}
